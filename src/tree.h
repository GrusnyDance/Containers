...
#ifndef _SRC_BTREE_H_
#define _SRC_BTREE_H_
...
struct STree {
  ...
  STree<Key> *left_;
  STree<Key> *right_;
  STree() : parent_(nullptr), left_(nullptr), right_(nullptr) {}
};
...
class Tree {
 ...
 public:
  Tree() : first_(nullptr), root_(nullptr), size_(0), comp_(Compare()) {
    fake_ = new STree<K>();
  }

  Tree(std::initializer_list<key_type> const &keys)
      : first_(nullptr), root_(nullptr), size_(0), comp_(Compare()) {
    fake_ = new STree<K>();
    operator=(keys);
  }
  ...
  Tree(Tree &&other)
      : first_(other.first_), root_(other.root_), size_(other.size_) {
    fake_ = new STree<K>();
    other.root_ = nullptr;
 ...
 protected:
  ...
  void pasteNewNode(node &root, node new_node) {
    ...
    while (tmp_parent != nullptr) {
      ...
      if (comp_(new_node->data_,
                tmp_parent->data_)) {    // new node < parent node
  ...
  node search(const key_type key) const { return searchRecurs(root_, key); }
